...
var top, left, height;
var time = new Date();
var linksContainer = document.getElementsByClassName(
  "grind-links-container"
)[0];
var linksLength = linksContainer.childElementCount - 1;
var timerShowProperties;
var mainLinksProperties = [
...
function createLines() {
  ...
  setTimeout(function() {
    newLine.style =
      ...
      "px; left: " +
      left +
      "px; transform: rotate(20deg) translateY(255px); z-index: 2";
  }, 20);
  setTimeout(function() {
    newLine.outerHTML = "";
  }, 2000);
...
setInterval(function() {
  if (ticking.style.color == "rgba(40, 40, 40, 0)") {
    ticking.style.color = "rgb(255,255,255)";
  } else {
    ticking.style.color = "rgba(40, 40 ,40, 0)";
  }
}, 666);
if (time.getHours() > 12) {
...
for (var i = 0; i <= linksLength - 1; i++) {
  ...
  links[i].onclick = function() {
    window.location =
      mainLinksProperties[
        Array.prototype.indexOf.call(this.parentNode.children, this)
      ][0];
  };
  ...
  links[i].addEventListener("mouseenter", function() {
    if (this.getAttribute("class").includes("instagram"))
      linesColor = "linear-gradient(to bottom,purple,orange,yellow)";
...
function openNewLinkModal() {
  document.getElementsByClassName("modal-background")[0].style.display =
    "block";
  setTimeout(function() {
    document.getElementsByClassName("add-new-link")[0].style.transform =
      "translateX(-50%) translateY(55vh)";
  }, 20);
}
document.getElementsByClassName("new")[0].onclick = function() {
  //open modal
  openNewLinkModal();
};
...
function addNewLink() {
  ...
  let newLinkName = document
    .getElementById("enter-name-input")
    .value.toLowerCase();
  ...
  newLinkInnerDivOptionsImg.setAttribute(
    "src",
    "https://img.icons8.com/metro/26/000000/more.png"
  );
  ...
  linksContainer.insertBefore(
    newLinkAnchor,
    linksContainer.childNodes[linksLength * 2]
  );
  ...
  newLinkAnchor.onclick = function() {
    window.location =
      mainLinksProperties[
        this.children[0]
          .getAttribute("id")
          .charAt(this.children[0].getAttribute("id").length - 1)
      ][0];
  };
  closeAddNewLinkModal();
  linksLength++;
...
document.getElementsByClassName(
  ...
  //add element, modal
  if (
    document.getElementsByClassName("add-new-link-buttons")[1].innerHTML ===
    "Save"
  ) {
    ...
    mainLinksProperties[elId][1] = document.getElementById(
      "enter-name-input"
    ).value;
    document.getElementById(
      "main-link-" + elId.toString()
    ).nextSibling.innerHTML = mainLinksProperties[elId][1];
    closeAddNewLinkModal();
  } else {
    addNewLink();
  }
};
var changeColorModalPopup_DivContainer = document.getElementsByClassName("modal-background-2")[0];
function changeColorModalPopup(){
  changeColorModalPopup_DivContainer.style.display = "block";
  setTimeout(function(){
    changeColorModalPopup_DivContainer.firstChild.nextSibling.style.top = "25%";
  },20) 
}
function closeColorModalPopup(){
  changeColorModalPopup_DivContainer.style.display = "none";
  changeColorModalPopup_DivContainer.firstChild.nextSibling.style.top = "-27%";
  document.getElementsByClassName("change-color-modal-input")[0].value = ""; 
}
changeColorModalPopup_DivContainer.onclick = function(event){
  if(event.target!==changeColorModalPopup_DivContainer.firstChild.nextSibling){
    if(event.target.parentNode!==changeColorModalPopup_DivContainer.firstChild.nextSibling){
      closeColorModalPopup();
    }
  }
}
document.getElementsByClassName("change-color-modal-input")[0].onkeypress = function(event){
  console.log(document.getElementsByClassName("change-color-modal-error")[0].style.display);
  if(document.getElementsByClassName("change-color-modal-error")[0].style.display == "block"){
    document.getElementsByClassName("change-color-modal-error")[0].style = "display: none !important";
  }
  if(event.keyCode==13&&this.value!==""){
    if(/(#[a-f0-9]{6})/gm.test(this.value)){
      changeColor(this.value);
      closeColorModalPopup();
      document.getElementsByClassName("change-color-modal-error")[0].style = "display: none !important";
    }
    else{
      document.getElementsByClassName("change-color-modal-error")[0].style = "display: block !important";
    }
  }
}
function changeColor(color){
  document.body.style.backgroundColor = color;
  topHeaderDiv.style.backgroundColor = color;
  document.getElementsByClassName("clock-outer")[0].style.backgroundColor = color;
  changeColorModalPopup_DivContainer.style.display = "none";
}
...
function showPropertiesModal(event, el) {
  ...
  document.getElementById("enter-url-input").value =
    mainLinksProperties[elId][0];
  document.getElementById("enter-name-input").value =
    mainLinksProperties[elId][1];
